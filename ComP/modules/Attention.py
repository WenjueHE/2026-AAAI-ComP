...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, y, mask_modality, mask=None):
        B, seq_len, C = x.shape
        c, dim = y.shape  # y shape: (c, dim)
        
        # Expand y to match batch dimension and add sequence dimension
        y = y.unsqueeze(0).expand(B, -1, -1)  # y shape: (B, c, dim)
        
        # Project and reshape query (from y)
        q = self.q(y).reshape(B, c, self.num_heads, -1).permute(0, 2, 1, 3)  # [B, heads, c, head_dim]
        
        # Project and reshape key/value (from x)
        k = self.k(x).reshape(B, seq_len, self.num_heads, -1).permute(0, 2, 1, 3)  # [B, heads, seq_len, head_dim]
        v = self.v(x).reshape(B, seq_len, self.num_heads, -1).permute(0, 2, 1, 3)  # [B, heads, seq_len, head_dim]

        q = q * self.scale
        attn = (q.float() @ k.float().transpose(-2, -1))  # [B, heads, c, seq_len]
...
class Block(nn.Module):
    ...
    def forward(self, x, first_stage, mask=None, modality=None, weight=None):
        if first_stage:
